...
//即时打印游戏界面
void interface(int x, int y, char flag[100][100]) {
    for (int i = 0; i <= x; i++) {
        for (int j = 0; j <= y; j++) {
            if (i == 0) {
                printf("%d ", j);
...
//进行探测
int detect(int x,int y,int* num,int bl, int mine[101][101], char flag[100][100]) {
    int a, b;
    if (bl == 2) printf("这个位置已经探测过了，请输入一个未探测过的位置:>");
    else if (bl == 3) printf("这个位置不在棋盘上,请重新输入:>");
    else printf("请输入你要探测的位置:>");
    scanf("%d %d", &a, &b);
    if (a <= x && b <= y && a > 0 && b > 0) {
...
void game(int x, int y, int nummine) {
    ...
    do {
        //test(x,y,mine);
        interface(x, y, flag); //打印游戏界面
        bl = detect(x,y,&num, bl, mine, flag); //进行扫雷行为
        system("CLS");//清空屏幕
        if (num == 0) break;
    } while (bl);
    if (bl) printf("*********扫雷完成**********\n");
